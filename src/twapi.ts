...
const taskPath = which_sync( 'task' );
...
export function add (task : Task, relPath : string ) {
    ...
    const added = JSON.parse(execFileSync( taskPath, [uuid,"export"] ).toString())[0];
...
function fromMatch  (match : MatchType[])  : Task{
    const status : string = <string>match[1] === 'x' ? "completed" : (<string>match[1] === '*' ? "deleted" : "pending");
    const rawItem : string = match[2] as string;
    const tags = findTags(rawItem);
    const desc = tags.length === 0 ? rawItem.trim() : rawItem.split('#')[0].trim();
...
export function toMD (task : Task)  {
  const chkBox = (task.status === "completed") ? "- [x] " : (task.status === "deleted" ? "- [*] " : "- [ ] ");
  ...
  */
  const doneTag = task.status === "completed" ? ` {ok:${lastTouch.toDate().getTime()}}` : '';
...
export function rem(id: string) {
    ...
    const output = execFileSync( taskPath, taskparm,{input: "yes\n"} ).toString();
  console.log(output);
  return output;
}
...
function setPath (old: Task,relpath : string) {
  ...
  if (!relpath){
    old.annotations = old.annotations.filter(n=>!n.description.startsWith("file="));
    return;
  }
  let hit = false;
...
export function upsertAll  (tasks: Task[], relPath : string, twApi : any)  {
  ...
  for (const t in tasks){
    ...
    if (ret.length > 1 && ret[ret.length - 1].uuid === ret[ret.length - 2].uuid){
...
export function entry(id:string, date : unknown) {
  ...
  if (date instanceof Date) {
    ...
    value = `${value}${myDate.getFullYear()}-${myDate.getMonth()+1}-${myDate.getDate()}`
  }
